...
use file_guardian::analyzers::external::{
    ...
    NormalizedScannerOccurrence, PreparedProtectedFile, PreparedScannerExecutable,
    PreparedScannerSandbox, ScannerAssignment, ScannerAssignmentSurface, ScannerCancellation,
    ScannerCompletion, ScannerKind, ScannerProtocolError, ScannerRunError, ScannerRunLimits,
    ScannerSandboxError, ScannerSandboxSpec, ScannerVersion, ScannerVersionRequirement,
};
...
async fn pre_cancelled_invocation_never_starts_the_sandbox() {
    ...
    write_executable_script(&scanner_path, 0);
    write_executable_script(&bubblewrap_path, 0);
...
int main(int argc, char **argv) {
  ...
  if (host < 0) return 91;
  close(host);
  if (open("/etc/passwd", O_WRONLY) >= 0) return 97;
...
#[tokio::test]
#[ignore = "live Gitleaks acceptance; requires explicitly supplied installed paths"]
async fn live_gitleaks_runs_through_the_production_sandbox() {
    let executable =
        std::env::var_os("FILE_GUARDIAN_LIVE_GITLEAKS").expect("set FILE_GUARDIAN_LIVE_GITLEAKS");
    let config = std::env::var_os("FILE_GUARDIAN_LIVE_GITLEAKS_CONFIG")
        .expect("set FILE_GUARDIAN_LIVE_GITLEAKS_CONFIG");
    let ignore = std::env::var_os("FILE_GUARDIAN_LIVE_GITLEAKS_IGNORE")
        .expect("set FILE_GUARDIAN_LIVE_GITLEAKS_IGNORE");
    let root = TempDir::new().unwrap();
    let input = root.path().join("input");
    let output = root.path().join("output");
    fs::create_dir(&input).unwrap();
    fs::create_dir(&output).unwrap();
    fs::write(input.join("sample.txt"), b"ordinary fixture\n").unwrap();
    fs::set_permissions(input.join("sample.txt"), fs::Permissions::from_mode(0o400)).unwrap();
    fs::set_permissions(&input, fs::Permissions::from_mode(0o500)).unwrap();
    fs::set_permissions(&output, fs::Permissions::from_mode(0o700)).unwrap();
    ...
    let scanner =
        PreparedScannerExecutable::discover(ScannerKind::Gitleaks, executable.as_os_str(), None)
            .unwrap();
    let config = PreparedProtectedFile::open(std::path::Path::new(&config)).unwrap();
    let ignore = PreparedProtectedFile::open(std::path::Path::new(&ignore)).unwrap();
    let sandbox = PreparedScannerSandbox::prepare(ScannerSandboxSpec {
        bubblewrap_executable: "/usr/bin/bwrap".into(),
        expected_bubblewrap_version: "0.11.1".to_owned(),
    })
    ...
    let result = ExternalScannerRunner::new(sandbox)
        .run(
            &scanner,
            ScannerVersionRequirement::new(
                ScannerVersion::new(8, 30, 0),
                ScannerVersion::new(9, 0, 0),
            ),
...
#[tokio::test]
#[ignore = "live TruffleHog acceptance; requires an explicitly supplied installed path"]
async fn live_trufflehog_runs_through_the_production_sandbox() {
    let executable = std::env::var_os("FILE_GUARDIAN_LIVE_TRUFFLEHOG")
        .expect("set FILE_GUARDIAN_LIVE_TRUFFLEHOG");
    let root = TempDir::new().unwrap();
    let input = root.path().join("input");
    let output = root.path().join("output");
    fs::create_dir(&input).unwrap();
    ...
    let scanner =
        PreparedScannerExecutable::discover(ScannerKind::Trufflehog, executable.as_os_str(), None)
            .unwrap();
    let sandbox = PreparedScannerSandbox::prepare(ScannerSandboxSpec {
        bubblewrap_executable: "/usr/bin/bwrap".into(),
        expected_bubblewrap_version: "0.11.1".to_owned(),
...
fn write_executable_script(path: &std::path::Path, marker: u8) {
